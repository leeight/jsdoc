...
function generate(title, docs, filename, opt_viewname) {
    var docData = {
        title: title,
        docs: docs
    };

    var viewname = opt_viewname || 'container.tmpl';
    
    var outpath = path.join(outdir, filename),
        html = view.render(viewname, docData);
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    for (var longname in helper.longnameToUrl) {
        if ( hasOwnProp.call(helper.longnameToUrl, longname) ) {
            ...
            if (classes.length) {
                generate('Class: ' + classes[0].name, classes,
                    helper.longnameToUrl[longname], 'classes.tmpl');
            }
